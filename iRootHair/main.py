...
def parse_args():
    ...
    parser.add_argument('--skeleton_bin_height', help='Bin size for sliding window down skeletoniozed root to calculate root midline (default = 300 px)', type=int, nargs='?', dest='skeleton_bin_height', default=100)
    ...
    parser.add_argument('--remove_small_lengths', help='Length threshold to remove small lengths from length list; sets length for a particular bin to 0 when below the value (default = 5 px)', type=int, nargs='?', dest='length_filt', default=5)
    parser.add_argument('--conv', help='The number of pixels corresponding to 1mm in the original input images (default = 255 px)', type=int, nargs='?', dest='conv', default=255)
...
def main():
    ...
    for img_path in tqdm(args.img):
        ...
        skeleton = Skeleton()
        clean_root = skeleton.extract_root(root_mask, args.conn, args.root_crit, args.factor)
        ...
        rotated_root_mask = skeleton.calc_rotation(med_x, med_y, clean_root)
        ...
        skeleton.generate_buffer_coords(rotated_root_mask)
        straight_mask = skeleton.straighten_image(init_mask)
        
        rt = Root(straight_mask)

        # final_root_mask, final_rh_mask = rt.motsu_threshold(straightened_image)

        final_root = rt.check_root_tip(args.conn)
        rt.find_root_tip()
        root_hairs = rt.trim_rh_mask(args.conn, args.crit, args.factor)
